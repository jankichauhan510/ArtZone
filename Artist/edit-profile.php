...
<body style="background-image: url('back.jpg'); height:100%; width:100%;">
    <div class="container" class="container" style="margin-top: 25px; padding: 30px; box-shadow: 0px 1px 36px 5px rgba(0, 0, 0, 0.28);border-radius: 5px;">
        <div class="row">
            <div class="col-sm-3">
            </div>
            <div class="col-sm-6">
                    ...
                    <div class="login_form">
                        ...
                        <br> <?php
                                if (isset($_POST['update_profile'])) {
                                    ...
                                    //$InterestedArts = $_POST['$InterestedArts'];
                                    $phone = $_POST['phone'];
                                    ...
                                    if (!isset($error)) {
                                        if ($file != "") {
                                            $stmt = mysqli_query($dbc, "SELECT image FROM  artist WHERE email='$email'");
                                            $row = mysqli_fetch_array($stmt);
                                            $deleteimage = $row['image'];
                                            unlink($folder . $deleteimage);
                                            move_uploaded_file($file, $folder . $new_image_name);
                                            mysqli_query($dbc, "UPDATE artist SET image='$new_image_name' WHERE email='$email'");
                                        }
                                        $result = mysqli_query($dbc, "UPDATE artist SET fname='$fname',lname='$lname',username='$username',InterestedArts='$InterestedArts',phone='$phone',address='$address' WHERE email='$email'");
                                        if ($result) {
                                            header("location:artistprofile.php?profile_updated=1");
                                        } else {
                        ...
                        <form method="post" enctype='multipart/form-data' action="">
                            ...
                            <!--<div class="form-group">
                                <div class="row">
                                    <div class="col-3">
                                        <label>Interested Art</label>
                                    </div>
                                    <div class="col">
                                        <input type="text" name="InterestedArts" value="<?php echo $InterestedArts; ?>" class="form-control">
                                    </div>
                                </div>
                            </div>-->
                            <div class="form-group">
